...
int GUI::mainFrame()
{
    ...
    try
    {
        ...
        auto buttonex = tgui::Button::create();
        buttonex->setRenderer(theme.getRenderer("Button"));
        buttonex->setPosition(800, 70);
        buttonex->setText("OK");
        buttonex->setSize(100, 30);
        buttonex->connect("pressed", [=](){ canvas->draw(circle); });
        gui.add(buttonex);
        while (window.isOpen())
        {
            sf::Event event;
